...
mod tests {
    ...
    fn banned_maker_not_resurrected_by_same_bond_discovery() {
        ...
        let now_ts = 1000;
        let mut book = OfferBook::default();
        book.insert_candidate(address.clone(), outpoint, Some(500), now_ts, false);
        book.mark_bad(&address);
        assert_eq!(book.makers[0].state, MakerState::Bad);
    ...
    fn banned_maker_reconsidered_on_bond_rotation() {
        ...
        let old_outpoint = OutPoint::new(Txid::from_slice(&[2; 32]).unwrap(), 0);
        let new_outpoint = Some(OutPoint::new(Txid::from_slice(&[3; 32]).unwrap(), 0));
        let now_ts = 1000;
        let mut book = OfferBook::default();
        book.insert_candidate(address.clone(), Some(old_outpoint), Some(500), now_ts, false);
        book.mark_bad(&address);
        assert_eq!(book.makers[0].state, MakerState::Bad);
    ...
    fn banned_maker_excluded_from_active_and_polling() {
        ...
        live.insert(address.clone(), None);

        assert!(book.makers_to_poll(now_ts + 3600, &live).is_empty());
    ...
    fn ban_maker_handle_persists_state() {
        ...
        let address = addr("bad-handle-maker");
        {
            ...
            book.insert_candidate(address.clone(), None, None, 1000, false);
        }
