...
use super::{
    error::MakerError,
    handlers::{
        incoming_matches_swap_amount, is_already_broadcast_error, ConnectionState, Maker,
        SwapPhase, MIN_CONTRACT_REACTION_TIME,
    },
};
#[cfg(feature = "integration-test")]
...
use bitcoind::bitcoincore_rpc::{
    jsonrpc::error::{Error as JsonRpcError, RpcError},
    Error as BitcoinRpcError,
};
...
fn process_taproot_contract<M: Maker>(
    ...
    // so require confirmations before proceeding. One wait for the whole batch:
    // they were broadcast together, so a per-tx deadline would just multiply it.
    maker.wait_for_txs_on_chain(&data.id, &incoming_txids, required_confirms)?;
    for j in 0..n {
    ...
    // #1018's zero guard stays even though shares no longer divide by the
    // total: a zero-incoming swap is refused here, not downstream.
