...
The caps exist so a stranger cannot make your node do unbounded work. The
feerate floor exists because Bitcoin nodes do not forward cheaper
transactions. Every place you can set these numbers rejects bad values.
Nothing silently corrects them.
...
- It tries to reach your `tx_count` first. If the maker's coins cannot
  ...
  swap never fails just because the split count comes out lower.
...
hop earns, the maker refuses the swap.

The maker also refuses a funding output that is already spent, or one that
belongs to another swap. Replayed data can never fund a second hop.
...
wallet. Keeping that link broken shapes every rule below.
...
5. Makers often come in cheaper than the worst case. That leaves a
   ...
   amount becomes extra miner fee on the payment transaction. That burn is
   the price of the payment staying unlinked.
